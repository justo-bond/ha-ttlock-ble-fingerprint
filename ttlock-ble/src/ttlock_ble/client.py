...
def _date_string_to_datetime(value: str) -> dt.datetime:
    """Convert a TTLock validity string to a naive datetime."""
    if len(value) == 10:
        ...
        raise ValueError(f"date must be YYMMDDHHmm or YYYYMMDDHHmm, got {value!r}")
...
class TTLockClient:
    ...
    async def add_fingerprint(
        ...
        *,
        ...
        """Scan and store a fingerprint, then apply its validity window.

        `start_date` and `end_date` accept `YYYYMMDDHHmm` or `YYMMDDHHmm`.
        The caller must be physically near the lock: after this command starts,
        place the same finger on the reader until the lock accepts it.
        """
        async with self._command_lock:
            await self._admin_login()
            fingerprint_number = await self._fingerprint_scan(scan_timeout)
            await self._fingerprint_ack_exchange(
                cmd.payload_fingerprint_update(fingerprint_number, start_date, end_date),
                "update_fingerprint",
            )
            return Fingerprint(
                fingerprint_number=fingerprint_number,
                start_date=_date_string_to_datetime(start_date),
                end_date=_date_string_to_datetime(end_date),
            )

    async def get_fingerprints(self) -> list[Fingerprint]:
        """Return fingerprint credentials stored on the lock."""
        async with self._command_lock:
            await self._admin_login()
            ...
            while True:
                frame = Frame.for_lock(
                    self.key.lockVersion,
                    cmd.CMD_FR_MANAGE,
                    cmd.payload_fingerprint_search(next_sequence),
                ).encrypt_data(self._aes_key)
                resp = await self._exchange(frame)
                plain = aes_decrypt(resp.data, self._aes_key)
                page, sequence, _battery = cmd.parse_fingerprint_search_response(plain)
                fingerprints.extend(item for item in page if isinstance(item, Fingerprint))
                if sequence in {-1, 0, next_sequence} or sequence in seen_sequences:
                    break
                seen_sequences.add(sequence)
                next_sequence = sequence & 0xFFFF
    ...
    async def update_fingerprint(
        self,
        fingerprint_number: str | int,
        *,
        start_date: str = "200001010000",
        end_date: str = "209912312359",
    ) -> None:
        """Update a fingerprint validity window."""
        async with self._command_lock:
            await self._admin_login()
            await self._fingerprint_ack_exchange(
                cmd.payload_fingerprint_update(fingerprint_number, start_date, end_date),
    ...
    async def delete_fingerprint(self, fingerprint_number: str | int) -> None:
        """Delete one fingerprint by its lock-local fingerprint number."""
        async with self._command_lock:
            await self._admin_login()
            await self._fingerprint_ack_exchange(
                cmd.payload_fingerprint_delete(fingerprint_number),
                "delete_fingerprint",
            )

    async def clear_fingerprints(self) -> None:
        """Wipe ALL fingerprints from the lock. There's no undo."""
        async with self._command_lock:
            await self._admin_login()
            await self._fingerprint_ack_exchange(
                cmd.payload_fingerprint_clear(),
                "clear_fingerprints",
            )
    ...
    async def _admin_login(self) -> None:
        """Authenticate admin-only operations on the current BLE session."""
        if self._admin_authenticated:
            return
        if not self.key.adminPs:
            raise TTLockError("This key does not include the admin password needed for biometrics")
        admin_frame = Frame.for_lock(
            self.key.lockVersion,
            cmd.CMD_CHECK_ADMIN,
            cmd.payload_check_admin(
                uid=self.key.uid,
                admin_ps=self.key.adminPs,
                lock_flag_pos=self.key.lockFlagPos,
            ),
        ).encrypt_data(self._aes_key)
        admin_resp = await self._exchange(admin_frame)
        admin_plain = aes_decrypt(admin_resp.data, self._aes_key)
        try:
            ps_from_lock = cmd.parse_check_admin_response(admin_plain)
        except (RuntimeError, ValueError) as exc:
            raise TTLockError(f"Admin authentication failed: {exc}") from exc
        if ps_from_lock <= 0:
            raise TTLockError(f"Admin authentication returned invalid psFromLock={ps_from_lock}")

        random_frame = Frame.for_lock(
            self.key.lockVersion,
            cmd.CMD_CHECK_RANDOM,
            cmd.payload_check_random(ps_from_lock, self.key.unlockKey),
        ).encrypt_data(self._aes_key)
        random_resp = await self._exchange(random_frame)
        random_plain = aes_decrypt(random_resp.data, self._aes_key)
        echo, status, data = cmd.parse_response_status(random_plain)
        log.info(
            "check_random response: cmd_echo=0x%02x status=%d data=%s",
            echo,
            status,
            data.hex(),
        )
        if status != cmd.RESPONSE_SUCCESS:
            raise TTLockError(
                f"Admin authentication rejected by checkRandom: "
                f"status={status:#x}, error={data.hex()}"
            )
        self._admin_authenticated = True

    async def _fingerprint_ack_exchange(self, payload: bytes, label: str) -> int | None:
        frame = Frame.for_lock(
            self.key.lockVersion, cmd.CMD_FR_MANAGE, payload
        ).encrypt_data(self._aes_key)
        resp = await self._exchange(frame)
        plain = aes_decrypt(resp.data, self._aes_key)
        try:
            battery = cmd.parse_fingerprint_ack_response(plain, label)
        except (RuntimeError, ValueError) as exc:
            raise TTLockError(f"Failed to {label}: {exc}") from exc
        log.info("%s response accepted; battery=%s", label, battery)
        return battery

    async def _fingerprint_scan(self, scan_timeout: float) -> str:
        """Start fingerprint enrollment and wait for the final fingerprint number."""
        frame = Frame.for_lock(
            self.key.lockVersion,
            cmd.CMD_FR_MANAGE,
            cmd.payload_fingerprint_add(),
        ).encrypt_data(self._aes_key)
        ...
        try:
            ...
            while True:
                ...
                if remaining <= 0:
                    raise TTLockError(
                        f"Timed out waiting {scan_timeout:.1f}s for fingerprint scan"
                    )
                try:
                ...
                plain = aes_decrypt(resp.data, self._aes_key)
                try:
                    battery, scan_status, fingerprint_number = (
                        cmd.parse_fingerprint_add_response(plain)
                    )
                except (RuntimeError, ValueError) as exc:
                    raise TTLockError(f"Fingerprint scan failed: {exc}") from exc
                log.info(
                    "fingerprint scan status=0x%02x battery=%s fp=%s",
                    scan_status,
                    battery,
                    fingerprint_number,
                )
                if (
                    scan_status == cmd.FR_STATUS_ADD_SUCCESS
                    and fingerprint_number is not None
                ):
                    return fingerprint_number
                if scan_status in {
                    cmd.FR_STATUS_ENTER_ADD_MODE,
                    cmd.FR_STATUS_PROGRESS,
                    cmd.FR_STATUS_RECEIVE_TEMPLATE,
                }:
                    continue
                raise TTLockError(f"Unexpected fingerprint scan status={scan_status:#x}")
        finally:
            self._waiting_for_response -= 1
